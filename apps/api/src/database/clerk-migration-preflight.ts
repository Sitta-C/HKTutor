...
const LEGACY_AGGREGATE_QUERY = `
  ...
  SELECT
    ...
    COUNT(*) FILTER (WHERE (u.id IN (SELECT id FROM known_synthetic_tutors) OR u.email IN (SELECT email FROM known_synthetic_tutors)) AND NOT (u.role = 'tutor' AND EXISTS (SELECT 1 FROM known_synthetic_tutors k WHERE k.id = u.id AND k.email = u.email))) AS "invalidSyntheticTutorCount",
