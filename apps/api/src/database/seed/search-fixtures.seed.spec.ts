...
function createClient(role: Role = Role.TUTOR) {
  ...
  const userIdsByEmail: Record<string, string> = {
    'mali@s1t20.hktutor.invalid': 'mali-id',
    'kiet@s1t20.hktutor.invalid': 'kiet-id',
    'niran@s1t20.hktutor.invalid': 'niran-id',
    'pim@s1t20.hktutor.invalid': 'pim-id',
  };
...
describe('seedTutorSearchFixtures', () => {
  it('seeds the exact-match, boundary, subject-mismatch, and grade-mismatch matrix', async () => {
    ...
    expect(
      ...
      { userId: 'mali-id', ratingAverage: '4.40', reviewCount: 18 },
      { userId: 'kiet-id', ratingAverage: '4.00', reviewCount: 10 },
      { userId: 'niran-id', ratingAverage: '4.70', reviewCount: 12 },
      { userId: 'pim-id', ratingAverage: '4.60', reviewCount: 15 },
    ]);
