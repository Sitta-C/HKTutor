...
| ---------------------------------------------------- | ---------------------------- | ---------------------------- |
| `POST /api/v1/auth/register`                         | `RegisterAuthDoc`            | 201, 400, 503                |
...
| Q3  | US11-1 | Registration does not disclose an existing email     | `apps/api/test/unit/auth/auth.service.spec.ts` — `returns the same generic result for an existing email without changing account state`                                                                                                                                                                                                     | Pass   |
...
| Q13 | US12-1 | Listing publish and status transitions               | `apps/api/test/unit/tutors/tutors.service.spec.ts` — `publishes a listing for an eligible tutor`; `apps/api/test/unit/tutors/tutors.service.spec.ts` — `returns 403 when a pending or rejected tutor tries to publish`; `apps/api/test/unit/tutors/tutors.service.spec.ts` — `restores a listing to draft and clears its publish timestamp` | Pass   |
...
| Q20 | US1-1  | Search exposes verified published tutors only        | `apps/api/test/unit/tutors/public-tutors.service.spec.ts` — `returns only public fields for published search results`                                                                                                                                                                                                                       | Pass   |
...
| Q22 | US12-3 | Booking created for an active student on a free slot | `apps/api/test/unit/bookings/bookings.service.spec.ts` — `creates a booking for a verified tutor when the student is active and the slot is available`                                                                                                                                                                                      | Pass   |
...
| I3  | S1-T28 traceability     | Minor    | Implementation previously let pending tutors publish and appear in discovery even though workbook US1-1 requires verified tutors.                                | It, Model  | Closed | Shared public tutor allowlist and publish authorization now require `VERIFIED`                       |
