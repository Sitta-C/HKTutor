...
| Q3  | US11-1 | Repeat registration rejects duplicate email          | `apps/api/test/unit/auth/auth.service.spec.ts` — `rejects repeated registration without rotating its token`                                                                                                                                                                   | Pass         |
...
| Q13 | US12-1 | Listing publish and status transitions               | `apps/api/test/unit/tutors/tutors.service.spec.ts` — `publishes a listing for an eligible tutor`; `apps/api/test/unit/tutors/tutors.service.spec.ts` — `restores a listing to draft and clears its publish timestamp`                                                         | Pass, see I3 |
