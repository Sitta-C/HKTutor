...
| `bio`             | String  | Yes      | Trimmed, 1–2,000 characters |
| `experienceYears` | Integer | Yes      | Integer, 0 or greater       |

- Behavior: create or update the authenticated tutor's profile. Client input cannot set
  verification, rating, review, or lifecycle metadata fields.
- Output `200`: complete saved `TutorProfile`.
- Errors: `400` field validation, `401` unauthenticated, `403` wrong role.

The limits above are enforced by the profile DTO and published through Swagger; Prisma remains the
storage layer and does not replace request validation.
...
| ------------------- | -------------------------------- | -------- | ---------------------------------------------- |
| `publicationStatus` | `DRAFT \| PUBLISHED \| ARCHIVED` | Yes      | Publishing requires a `VERIFIED` tutor profile |

- Output `200`: complete updated `TeachingListing`.
- Errors: `400` invalid transition, `401`, `403`, `404 LISTING_NOT_FOUND`,
  `409 TUTOR_NOT_VERIFIED`.

An `ARCHIVED` listing can return to `DRAFT` without verification and can return to `PUBLISHED` only
when the tutor is `VERIFIED`. Restoring to `DRAFT` clears `publishedAt`; publishing sets a fresh
publication timestamp. The frontend exposes only these transitions.
