...
| Next session          | Student label approved by the API, subject, grade, Bangkok time and status         | Own `Booking` + listing/catalog + slot; student relation            | Open detail; omit the participant label until a privacy-approved Student display field exists                                 |
...
| Booking requests list | Allowed student label, subject/level, date/time, amount and PENDING badge          | Owner-scoped Booking join                                           | Confirm/Reject only in Sprint 2 US12-4; re-read state on conflict; do not expose controls merely because a badge says pending |
...
| No student display profile                  | `User` lacks name/photo; `AuthUser` is only id/email/role                                                                                      | Omit the participant label until an API-approved private display field exists; public email is not a Tutor-facing fallback                        |
